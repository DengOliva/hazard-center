...
import {
  ...
  requireArg,
  saveBlobToFile,
} from "file:///C:/Users/Yucon/.codex/plugins/cache/openai-primary-runtime/presentations/26.723.12215/skills/presentations/container_tools/artifact_tool_utils.mjs";
...
function zipNames(pptxPath) {
  return String(runCapture("C:\\Program Files\\7-Zip\\7z.exe", ["l", "-slt", pptxPath], { encoding: "utf8" }))
    .split(/\r?\n/)
    .filter((line) => line.startsWith("Path = "))
    .map((line) => line.slice(7).replaceAll("\\", "/"))
    .filter((name) => name && path.resolve(name) !== path.resolve(pptxPath));
}

function readZipText(pptxPath, entryName) {
  return Buffer.from(runCapture("C:\\Program Files\\7-Zip\\7z.exe", ["e", "-so", pptxPath, entryName])).toString("utf8");
}

async function copyZipEntry(pptxPath, entryName, targetPath) {
  await fs.mkdir(path.dirname(targetPath), { recursive: true });
  await fs.writeFile(targetPath, Buffer.from(runCapture("C:\\Program Files\\7-Zip\\7z.exe", ["e", "-so", pptxPath, entryName])));
}
